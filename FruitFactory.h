	...
	static Fruit *buildFruit(FruitType type) 
	{
		switch (type) 
		{
			case NONE: 
				return 0;
			case APPLE: 
				return new Fruit(S_APPLE_TILE, S_APPLE_TEXTURE, S_APPLE_TILE);
			case ORANGE: 
				return new Fruit(S_ORANGE_TILE, S_ORANGE_TEXTURE, S_ORANGE_TILE);
			case PEAR: 
				return new Fruit(S_PEAR_TILE, S_PEAR_TEXTURE, S_PEAR_TILE);
			case CHERRY: 
				return new Fruit(S_CHERRY_TILE, S_CHERRY_TEXTURE, S_CHERRY_TILE);
			default:
				return 0;
		}
	}
