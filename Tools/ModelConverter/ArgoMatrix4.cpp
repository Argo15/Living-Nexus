...
ArgoMatrix4::ArgoMatrix4()
{
	this->identity();
}

ArgoMatrix4::ArgoMatrix4(float v)
{
	for (int i=0;i<16;i++)
		data[i]=v;
}
...
ArgoMatrix4::ArgoMatrix4(ArgoMatrix4 &m)
{
	*this=m;
}

ArgoMatrix4::ArgoMatrix4(ArgoMatrix3 &m)
{
	*this=m;
}

float ArgoMatrix4::operator [](unsigned int i)
{
	return data[i];
}

ArgoMatrix4& ArgoMatrix4::operator =(ArgoMatrix4 &m)
{
	for (int i=0;i<16;i++)
		data[i]=m[i];
	return *this;
...
ArgoMatrix4& ArgoMatrix4::operator =(ArgoMatrix3 &m)
{
	this->identity();
	data[0]=m[0];
	data[1]=m[1];
...
ArgoMatrix4	&ArgoMatrix4::operator *= (float f)
{
	for (int i=0;i<16;i++)
		data[i]=data[i]*f;
	return *this;
...
bool ArgoMatrix4::operator ==(ArgoMatrix4 &m)
{
	for (int i=0;i<16;i++)
		if (data[i]!=m.data[i])
			return false;
...
ArgoMatrix4 ArgoMatrix4::operator /(ArgoMatrix4 &m)
{
	ArgoMatrix4 inv = this->inverse();
	ArgoMatrix4 ret = m*inv;
	return ret;
}
...
ArgoVector4 ArgoMatrix4::operator *(ArgoVector4 &v)
{
	ArgoVector4 ret;
	ArgoVector4 row[4];
...
ArgoMatrix4 &ArgoMatrix4::identity()
{
	for(int i=0;i<16;i++)
		data[i]=0;
	data[0]=1.0;
...
ArgoMatrix4 &ArgoMatrix4::transpose()
{
	ArgoMatrix4 m;
	m.data[0]=data[0];
	m.data[1]=data[4];
...
ArgoMatrix4 &ArgoMatrix4::inverse()
{
	// using RReF
	/*| 0  4  8   12 |
	  | 1  5  9   13 |
