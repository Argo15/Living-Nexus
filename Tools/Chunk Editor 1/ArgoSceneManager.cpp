...
void ArgoSceneManager::AddPhysicsShape(PhysicsShape *physicsShape)
{
	AddActor(physicsShape);
	physicsShapes[*physicsShape->getName()]=physicsShape;
}
...
void ArgoSceneManager::DrawScene(ArgoFrustum *frustum, GLSLProgram *program)
{
	...
	if (ArgoGraphicsSettings::Instance()->editMode == 0 || ArgoGraphicsSettings::Instance()->editMode == 1) {
			...
			if (frustum->isInFrustum(actor->getTranslateV(),actor->getScaledRadius())){
				glPushMatrix();
					...
					if (ArgoGraphicsSettings::Instance()->shadersEnabled()) {
						if (!(!ArgoGraphicsSettings::Instance()->physicsEnabled() && actor->isPhysics())) {
	...
	} else if (ArgoGraphicsSettings::Instance()->editMode == 2) {
		chunk->drawChunk(program);
	}
}
...
void ArgoSceneManager::copySelected()
{
	if (selectedActor == 0)
		return;
	ArgoObject *newObject = new ArgoObject();
	*newObject = *(objects[*selectedActor]);
	newObject->setName(new QString("Copy"));
	AddObject(newObject);
}