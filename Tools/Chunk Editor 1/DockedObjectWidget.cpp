	...
	: QWidget(0)
{
	...
	connect(physicsTypeCmb, SIGNAL(activated(QString)),this, SLOT(physicsTypeChanged(QString)));
...
void DockedObjectWidget::createFromObject(ArgoObject *object)
{
	...
	currentObject = object->getName();
	numTrianglesLbl->setText(QString::number(ArgoModelManager::Instance()->getModel(*object->getModel())->getNumTriangles()));
	physicsTrianglesLbl->setText(QString::number(ArgoModelManager::Instance()->getModel(*object->getModel())->getNumTriangles()));
	physicsTypeCmb->setCurrentIndex(object->getRigidBody()->type);
}
...
void DockedObjectWidget::physicsTypeChanged(QString type) {
	ArgoRigidBody *body = ArgoSceneManager::Instance()->getObject(*currentObject)->getRigidBody();
	body->type = (BodyType)physicsTypeCmb->findText(type);
}