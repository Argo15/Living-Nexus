...
void ArgoSaveFile::createSaveFromScene()
{
	...
	for (iobject = objectMap->begin(); iobject != objectMap->end(); ++iobject) {
		...
		strcpy(bodies[i].objectName,qPrintable(*object->getName()));
		objects[i].bodyType = object->getRigidBody()->type;
	...
	numPhysicsShapes = physicsMap->size();
	physicsShapes = new SavePhysics[numObjects];
	i=0;
	for (iphysics = physicsMap->begin(); iphysics != physicsMap->end(); ++iphysics) {
		PhysicsShape *physics = iphysics.value();
		strcpy(physicsShapes[i].name,qPrintable(*physics->getName()));
		physicsShapes[i].rotation[0] = physics->getRotate()[0];
		physicsShapes[i].rotation[1] = physics->getRotate()[1];
		physicsShapes[i].rotation[2] = physics->getRotate()[2];
		physicsShapes[i].rotation[3] = physics->getRotate()[3];
		physicsShapes[i].scale[0] = physics->getScale()[0];
		physicsShapes[i].scale[1] = physics->getScale()[1];
		physicsShapes[i].scale[2] = physics->getScale()[2];
		physicsShapes[i].translation[0] = physics->getTranslate()[0];
		physicsShapes[i].translation[1] = physics->getTranslate()[1];
		physicsShapes[i].translation[2] = physics->getTranslate()[2];
		physicsShapes[i].physicsType = physics->getType();
		i++;
	}
...
void ArgoSaveFile::createSceneFromSave(float version)
{
	...
	for (int i=0; i<numObjects; i++){
		...
		object->getRigidBody()->type = objects[i].bodyType;
	...
	for (int i=0; i<numPhysicsShapes; i++){
		PhysicsShape *physics = new PhysicsShape(physicsShapes[i].physicsType,QString(physicsShapes[i].name));
		physics->setTranslate(physicsShapes[i].translation[0],physicsShapes[i].translation[1],physicsShapes[i].translation[2]);
		physics->setRotate(ArgoQuaternion(physicsShapes[i].rotation[0],physicsShapes[i].rotation[1],physicsShapes[i].rotation[2],physicsShapes[i].rotation[3]));
		physics->setScale(physicsShapes[i].scale[0],physicsShapes[i].scale[1],physicsShapes[i].scale[2]);
		ArgoSceneManager::Instance()->AddPhysicsShape(physics);
	}
...
void ArgoSaveFile::saveScene(const char *filename)
{
	...
	{ 
		...
		file.write((char*)&numPhysicsShapes,sizeof(numPhysicsShapes));
		file.write((char*)physicsShapes,sizeof(SavePhysics)*numPhysicsShapes);

		file.write((char*)&numRigidBodies,sizeof(numRigidBodies));
		for (int i=0; i<numRigidBodies; i++){
			file.write((char*)&bodies[i].objectName[0],sizeof(char)*100);
			file.write((char*)&bodies[i].numTriangles,sizeof(bodies[i].numTriangles));
			file.write((char*)&bodies[i].triangles[0],sizeof(float)*9.0*bodies[i].numTriangles);
		}
...
void ArgoSaveFile::openScene(const char *filename)
{
	...
	if (file.is_open())
	{
		...
		if (version > 0.299) {
			...
			file.read((char*)&numPhysicsShapes,sizeof(numPhysicsShapes));
			physicsShapes = new SavePhysics[numPhysicsShapes];
			file.read((char*)physicsShapes,sizeof(SavePhysics)*numPhysicsShapes);
		}
		/*if (version > 0.399) {
