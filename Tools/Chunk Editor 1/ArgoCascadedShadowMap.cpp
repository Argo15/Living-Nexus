...
ArgoCascadedShadowMap::ArgoCascadedShadowMap(int size, float slice1, float slice2, float slice3)
{
	for (int i=0;i<4;i++) {
		shadowMaps[i] = new ArgoDepthFBO(size,size);
	}
	slices[0] = slice1;
	slices[1] = slice2;
	slices[2] = slice3;
...
void ArgoCascadedShadowMap::buildShadowMaps(ArgoCamera *camera, ArgoView *view, ArgoFrustum *frustum)
{
	float slice[] = {0.0, slices[0], slices[1], slices[2], 1.0};

	for (int i=0; i<4; i++) {
		ArgoCamera *lightCamera = createLightCamera(slice[i],slice[i+1],camera,view);
		ArgoView *lightView = createLightView(slice[i],slice[i+1],camera,lightCamera,view,frustum);
		ArgoFrustum *lightFrustum = new ArgoFrustum();
...
ArgoCamera *ArgoCascadedShadowMap::createLightCamera(float slice1, float slice2, ArgoCamera *camera, ArgoView *view)
{
	...
	ArgoCamera *lightCamera = new ArgoCamera();
...
ArgoView *ArgoCascadedShadowMap::createLightView(float slice1, float slice2, ArgoCamera *camera, ArgoCamera *lightCamera, ArgoView *view, ArgoFrustum *frustum)
{
	...
	ArgoView *lightView = new ArgoView();
	lightView->set2D(left,right,down,up,0.1,100);
	return lightView;
}

void ArgoCascadedShadowMap::drawObjects(ArgoFrustum *frustum)
{
		...
		for (it = objects->begin(); it != objects->end(); ++it){
			...
			if (frustum->isInFrustum(object->getTranslateV(),object->getScaledRadius())){
				glPushMatrix();
					object->transform();
					ArgoModelManager::Instance()->DrawModel(*(object->getModel()));
				glPopMatrix();
			}
		}
}

void ArgoCascadedShadowMap::sendToShader(GLSLProgram *glslProgram, ArgoView *viewport)
{
	glPushMatrix();
