...
ArgoTgaTexture::ArgoTgaTexture() : m_pImageData(NULL)
{
	type = "Diffuse";
}

ArgoTgaTexture::~ArgoTgaTexture()
{
	Release();
}

void ArgoTgaTexture::SwapRedblue()
{
	switch (m_colorDepth)
	{
	case 32:
...
bool ArgoTgaTexture::load(const char *filename)
{
	...
	delete [] m_pImageData;
	int start = QString(filename).indexOf(QString("Data/"));
	location=QString(filename).mid(start);
...
bool ArgoTgaTexture::FlipVertical()
{
	if (!m_pImageData)
		return false;
