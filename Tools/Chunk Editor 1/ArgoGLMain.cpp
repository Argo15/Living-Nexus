...
void ArgoGLMain::update(float fps)
{
	if (fps>0){
		if (e_key)
			camera->moveUp(5.0/fps);
		if (q_key)
			camera->moveDown(5.0/fps);
		camera->move(wsad,60.0/fps);
	}
...
void ArgoGLMain::render()
{
	...
	gBuffers->bind();
		...
		if (ArgoGraphicsSettings::Instance()->editMode != 2)
			myGrid->draw();
...
void ArgoGLMain::keyPressEvent(QKeyEvent * event) 
{
	...
	case Qt::Key_C:
		if (ctrl) {
			ArgoSceneManager::Instance()->copySelected();
		}
		break;
	default:
		break;
...
void ArgoGLMain::mousePressEvent(QMouseEvent *event)
{
	...
	if (event->buttons() & Qt::LeftButton){
		...
		if (ArgoGraphicsSettings::Instance()->editMode == 2)
			ArgoSceneManager::Instance()->getChunk()->update(x,y,event->buttons() & Qt::LeftButton);
	}
}
