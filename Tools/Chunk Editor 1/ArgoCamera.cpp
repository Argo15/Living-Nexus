...
void ArgoCamera::zoom(float amt){
	distance-=0.1*amt;
	if (distance<0.1)
		distance=0.1;
...
void ArgoCamera::updateFromDistance()
{
...
void ArgoCamera::mouseRotate(int newX,int newY){
	if (oldX==-1){
...
void ArgoCamera::move(bool wsad[4],float speed)
{
	if (wsad[0])
...
void ArgoCamera::moveForward(float speed)
{
...
void ArgoCamera::movebackward(float speed)
{
	eyePos[0]+=cos(hAngle)*speed;
	eyePos[2]-=sin(hAngle)*speed;
	lookAt[0]+=cos(hAngle)*speed;
...
void ArgoCamera::moveLeft(float speed)
{
...
void ArgoCamera::moveRight(float speed)
{
	eyePos[2]-=cos(hAngle)*speed;
	eyePos[0]-=sin(hAngle)*speed;
	lookAt[2]-=cos(hAngle)*speed;
...
void ArgoCamera::moveUp(float speed)
{
	eyePos[1]+=speed;
	lookAt[1]+=speed;
}

void ArgoCamera::moveDown(float speed)
{
	eyePos[1]-=speed;
	lookAt[1]-=speed;
}