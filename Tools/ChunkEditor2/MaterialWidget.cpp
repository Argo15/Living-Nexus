...
{
	Material *curMat = Root::materialManager->getMaterial(matName);
	...
	int texIndex=0;
	int normIndex=0;
	std::map<std::string,Texture *> *textures = Root::textureManager->getTextures();
...
{
	return Root::materialManager->getMaterial(matCmb->currentText().toStdString());
}
...
void MaterialWidget::addNewMaterial()
{
	...
	*newMat = *defaultMat;
	...
	string name = Root::materialManager->AddMaterialSafe(newMat);
	refresh(name);
...
void MaterialWidget::nameChanged(QString name)
{
	...
	{
		newName = Root::materialManager->renameMaterial(currentName, name.toStdString());
