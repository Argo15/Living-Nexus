...
void Label::draw()
{
	GuiElement::draw();
	glPushMatrix();
		float nTextScaleX = 1.0f/gFontManager->getFont()->getPhraseWidth(m_sText, m_nTextPosition[0], m_nTextSize[1]);
		...
		gFontManager->getFont()->draw(m_sText, m_nTextPosition[0]/nPaddedTextScaleX, m_nTextPosition[1], m_nTextSize[1]);
	glPopMatrix();
}
